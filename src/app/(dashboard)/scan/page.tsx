...
import { notifyScanFailed } from '@/lib/notificationService';
...
export default function ScanPage() {
  ...
  const handleTextSubmit = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (user?.uid) {
        notifyScanFailed(user.uid, err instanceof Error ? err.message : undefined);
      }
    ...
    <div className="max-w-6xl mx-auto space-y-8 px-4">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#E5E7EB] tracking-tight">AI Notice Scanner</h1>
        ...
        {scanState === 'idle' && (
          ...
          >
            <div className="space-y-5">
              {/* Tab Switcher */}
              <div className="flex items-center p-1 rounded-xl bg-[#111827] border border-[#1F2937] w-fit">
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {recentNotices.map((notice) => (
                    <motion.div
                      ...
                      className={cn(
                        'group relative cursor-pointer p-4 rounded-xl border border-[#1F2937] bg-[#111827]',
                        'hover:bg-[#111827]/80 hover:border-[#3B82F6]/30 hover:shadow-lg',
                    ...
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2 mb-2">
                          ...
                          <button
                            ...
                            className="text-[#9CA3AF]/60 hover:text-rose-400 p-1 rounded-md hover:bg-[#0B0F19] opacity-0 group-hover:opacity-100 transition-all duration-200"
                            aria-label="Delete scan record"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <h4 className="text-sm font-semibold text-[#E5E7EB] line-clamp-1 group-hover:text-[#3B82F6] transition-colors">
                          {notice.title}
                        </h4>
                      ...
                      <div className="flex items-center justify-between text-[10px] text-[#9CA3AF]/70 font-medium pt-2 border-t border-[#1F2937] mt-2">
                        <span className="flex items-center gap-1">
                          <FileText className="w-3 h-3 text-[#9CA3AF]/60" />
