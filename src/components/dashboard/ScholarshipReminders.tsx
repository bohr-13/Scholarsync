      ...
      <div>
        ...
        {isLoading ? (
          ...
          <div className="py-8 text-center">
            <p className="text-xs text-[var(--text-secondary)]">No matching scholarships found right now.</p>
          </div>
        ) : (
          <div className="space-y-3.5">
            {activeScholarships.map((sch, i) => (
              <motion.div
                ...
                className="flex items-start justify-between p-3 rounded-xl border border-[var(--glass-border)] bg-[var(--glass-surface)] gap-3"
              >
                <div className="min-w-0 flex-1">
                  <h4 className="text-xs font-bold text-[var(--text-primary)] truncate">
                    {sch.name}
                  </h4>
                  <p className="text-[10px] text-[var(--text-secondary)] mt-0.5 truncate">{sch.provider}</p>
                  <div className="flex items-center gap-2 mt-1.5 text-[9px] text-[var(--text-secondary)] font-medium">
