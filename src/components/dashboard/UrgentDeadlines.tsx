...
function getFilteredTasks(tasks: Task[], isEmergencyActive: boolean) {
...
function Header({ isEmergencyActive, filteredCount }: { isEmergencyActive: boolean; filteredCount: number }) {
  return (
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center gap-2.5">
        ...
        <div>
          <h3 className="text-sm font-bold dark:text-white text-slate-900 tracking-tight">
            {isEmergencyActive ? 'Critical Deadlines Only' : 'Urgent Actions'}
          </h3>
          <p className="text-[10px] dark:text-slate-500 text-slate-400">
            {isEmergencyActive ? 'Calm workspace active' : 'Due within 48 hours'}
          </p>
        </div>
      </div>
      <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 animate-pulse">
        {filteredCount} {filteredCount === 1 ? 'Alert' : 'Alerts'}
      </span>
    </div>
  );
}
...
export default function UrgentDeadlines() {
  ...
  const filteredTasks = getFilteredTasks(tasks, emergency.isActive);
  ...
  return (
    ...
    >
      <div>
        ...
        <Header isEmergencyActive={emergency.isActive} filteredCount={filteredTasks.length} />
