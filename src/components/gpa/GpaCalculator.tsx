  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-bold text-[var(--text-primary)] tracking-tight">GPA Predictor</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <TiltCard className="p-5 lg:p-6" intensity={2}>
            <div className="flex items-center justify-between mb-5 border-b border-[var(--glass-border)] pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-[var(--text-primary)]">Semester SGPA Calculator</h3>
              </div>
              <button
                onClick={addSubject}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-[var(--glass-bg)] border border-[var(--glass-border)] hover:bg-[var(--glass-surface)] text-[var(--text-primary)] transition duration-200 cursor-pointer"
              >
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-12 gap-3 text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-wider px-2">
              ...
              <div className="space-y-2.5 max-h-[360px] overflow-y-auto pr-1">
                {subjects.map((sub) => (
                  <motion.div
                    ...
                    className="grid grid-cols-12 gap-3 items-center p-2 rounded-xl bg-[var(--glass-surface)] border border-[var(--glass-border)]"
                  >
                    {/* Name */}
                    <div className="col-span-6 md:col-span-7">
                      <input
                        ...
                        className="w-full text-xs bg-transparent border-0 border-b border-transparent focus:border-[#3B82F6]/50 text-[var(--text-primary)] p-1 focus:outline-none transition"
                    ...
                    <div className="col-span-3 md:col-span-2 flex items-center justify-center">
                      <select
                        ...
                        className="bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-lg text-xs text-center text-[var(--text-primary)] p-1.5 focus:outline-none focus:border-[#3B82F6] w-16"
                      >
                        {[1, 2, 3, 4, 5, 6].map((c) => (
                          <option key={c} value={c} className="bg-[var(--glass-bg)]">{c}</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="col-span-3 md:col-span-2 flex items-center justify-center">
                      <select
                        ...
                        className="bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-lg text-xs text-center text-[var(--text-primary)] p-1.5 focus:outline-none focus:border-[#3B82F6] w-16 font-semibold"
                      >
                        {Object.keys(GRADE_POINTS).map((g) => (
                          <option key={g} value={g} className="bg-[var(--glass-bg)]">{g}</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="col-span-12 md:col-span-1 flex items-center justify-center mt-2 md:mt-0">
                      <button
                        ...
                        className="p-1 rounded-lg text-[var(--text-secondary)] hover:text-[#EF4444] hover:bg-[#EF4444]/10 disabled:opacity-30 disabled:pointer-events-none transition cursor-pointer"
                      >
          ...
          {/* CGPA Forecast Panel */}
          <TiltCard className="p-5 lg:p-6" intensity={2}>
            <div className="flex items-center gap-2 mb-5 border-b border-[var(--glass-border)] pb-3">
              <TrendingUp className="w-5 h-5 text-indigo-400" />
              <h3 className="text-sm font-semibold text-[var(--text-primary)]">CGPA Forecast &amp; Predictor</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
              <div>
                <label className="text-[10px] uppercase font-bold text-[var(--text-secondary)] tracking-wider mb-2 block">
                  Current CGPA
                </label>
                <input
                  type="number"
                  ...
                  className="w-full bg-[var(--glass-surface)] border border-[var(--glass-border)] focus:border-[#3B82F6] rounded-xl text-xs text-[var(--text-primary)] p-2.5 focus:outline-none"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[var(--glass-surface)] border border-[var(--glass-border)] focus:border-[#3B82F6] rounded-xl text-xs text-[var(--text-primary)] p-2.5 focus:outline-none"
                >
                  <option value="0" className="bg-[var(--glass-bg)]">None (Freshman)</option>
                  {[1, 2, 3, 4, 5, 6, 7].map((s) => (
                    <option key={s} value={s} className="bg-[var(--glass-bg)]">Sem {s}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                >
                  {[4, 6, 8, 10].map((ts) => (
                    <option key={ts} value={ts} className="bg-[var(--glass-bg)]">{ts} Semesters</option>
                  ))}
                </select>
              </div>
            </div>
              ...
              {targetPrediction && (
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                    <div className="md:col-span-1 rounded-xl bg-[var(--glass-surface)] border border-[var(--glass-border)] p-3 text-center">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)] mb-1">
                        Needed Average SGPA
                      </p>
                      <h4 className="text-xl font-extrabold text-[#3B82F6]">
                        ...
                        return (
                          <div className={`p-3.5 rounded-xl border flex items-start gap-3 ${styles.bg}`}>
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-xs leading-relaxed text-[var(--text-primary)]/90">
        ...
        <div className="space-y-6">
          <TiltCard className="p-5 lg:p-6 flex flex-col items-center justify-center text-center h-full min-h-[300px]" intensity={3}>
            <div className="w-full flex items-center justify-center gap-1.5 mb-6">
              ...
              <h3 className="text-xs uppercase font-bold text-[var(--text-secondary)] tracking-wider">Estimated SGPA</h3>
            </div>

            <div className="relative w-44 h-44 flex items-center justify-center">
              {/* Outer Glow */}
              <div className="absolute inset-2 rounded-full bg-[var(--glass-surface)] border border-[var(--glass-border)] shadow-inner" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center space-y-0.5">
                <h2 className="text-3xl font-black text-[var(--text-primary)]">{sgpaResult.toFixed(2)}</h2>
                <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase tracking-wider">Estimated SGPA</p>
              </div>
            </div>
