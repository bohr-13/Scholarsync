...
export default function Features() {
  ...
  return (
    <section className="relative py-24 px-4 dark:bg-slate-950/20 bg-slate-50/50">
          ...
          {features.map((feat, i) => {
            ...
            return (
              <motion.div key={i} variants={cardVariants}>
                <TiltCard className="p-6 h-full flex flex-col items-start hover:border-white/10 dark:hover:border-white/10 dark:bg-white/[0.02]" glowColor={feat.glow} intensity={4}>
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${feat.color} flex items-center justify-center mb-5 border border-white/[0.05]`}>
                    <Icon className={`w-5 h-5 ${feat.iconColor}`} />
                  </div>
                  <h3 className="text-lg font-bold mb-3 dark:text-white text-slate-800">
                    {feat.title}
                  </h3>
                  <p className="text-sm dark:text-slate-400 text-slate-600 leading-relaxed flex-1">
                    {feat.description}
                  </p>
                </TiltCard>
              </motion.div>
