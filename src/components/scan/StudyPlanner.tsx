...
export default function StudyPlanner({ title, summary, deadline, onPlanChange }: StudyPlannerProps) {
  ...
  return (
    <div className="w-full rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl p-5 lg:p-6 mt-6">
      <AnimatePresence mode="wait">
        {!plan && !loading && (
          ...
          >
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 flex items-center justify-center mx-auto shadow-lg shadow-indigo-500/5">
              <Sparkles className="w-5 h-5 text-indigo-400 animate-pulse" />
            </div>
            <div className="max-w-md mx-auto space-y-1.5">
              <h4 className="text-sm font-semibold text-slate-200">Prepare Strategically</h4>
        ...
        {loading && (
          ...
          >
            ...
            <div className="text-center space-y-1">
              <p className="text-xs font-semibold text-slate-300">Crafting study roadmap...</p>
              <p className="text-[10px] text-slate-500 animate-pulse">Mapping daily focus &amp; active recall checks</p>
            </div>
          </motion.div>
        )}
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-white/[0.06] pb-4">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-slate-200">{plan.title}</h4>
                  <p className="text-[10px] text-slate-400">AI Study Strategist • 5 Days structured prep</p>
                </div>
              </div>
                ...
                {plan.days.map((d) => (
                  <button
                    ...
                    className={`w-7 h-7 rounded-lg text-xs font-bold transition-all ${
                      activeDay === d.day
                        ? 'bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 shadow-md shadow-indigo-500/5'
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {/* Daily focus banner */}
              <div className="md:col-span-1 rounded-xl bg-white/[0.01] border border-white/[0.04] p-4 flex flex-col justify-between">
                <div>
                  ...
                  <h5 className="text-sm font-semibold text-slate-200 mb-2 leading-snug">
              ...
              <div className="md:col-span-2 space-y-3">
                <div className="flex items-center gap-1.5 text-xs text-slate-400 mb-1 px-1 font-medium">
                      ...
                      >
                        <button
                          className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 mt-0.5 border transition-all ${
                            isDone ? 'bg-emerald-500/15 border-emerald-500/30' : 'border-white/20 group-hover:border-white/40'
                          }`}
                        >
                          {isDone && <CheckCircle2 className="w-3 h-3 text-emerald-400" />}
                        </button>
                        <p className={`text-xs leading-relaxed transition-all ${isDone ? 'line-through text-slate-500' : 'text-slate-300'}`}>
                          {task}
                        </p>
                      </div>
                    );
