...
import {
  notifyTodoListCreated,
  notifyTaskCompleted,
  notifyAiStudyPlanGenerated,
...
export function useTodos(): UseTodosReturn {
  ...
  const createList = useCallback(
    async (
      ...
      try {
        ...
        if (id && user?.uid) {
          if (type === 'ai-plan') {
            notifyAiStudyPlanGenerated(user.uid, title);
          } else {
            notifyTodoListCreated(user.uid, title);
          }
        }
        return id;
      } catch (err) {
  ...
  const toggleTaskComplete = useCallback(
    async (listId: string, taskId: string): Promise<void> => {
      ...
      try {
        await fsUpdateTodoList(user.uid, listId, { tasks: updatedTasks });
        if (completedTaskText && user?.uid) {
          notifyTaskCompleted(user.uid, completedTaskText);
        }
      } catch (err) {
