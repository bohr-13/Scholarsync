...
export async function addNotification(
  ...
  const ref = await addDoc(userCollection(userId, 'notifications'), payload);
  return ref.id;
}
...
export async function markNotificationRead(
  ...
  await updateDoc(ref, { isRead: true, _updatedAt: serverTimestamp() });
}
...
export async function markAllNotificationsRead(
  ...
  const q = query(
    userCollection(userId, 'notifications'),
    // Limit to unread only for efficiency
  );
  const snapshot = await getDocs(q);
  const batchWrites = snapshot.docs
    .filter((d) => d.data().isRead === false)
    .map((d) => updateDoc(doc(db, 'users', userId, 'notifications', d.id), {
      isRead: true,
      _updatedAt: serverTimestamp(),
    }));
...
/** Subscribe to real-time notification updates, latest first */
